...
TARGET_START_DATETIME = "2026-08-15T15:35:05"

# ✅ 1개의 엑셀 파일 저장을 위한 경로 및 파일명 설정 (.xlsx 로 변경)
SAVE_DIR = r"C:\Users\원영이\Desktop\ATL AUTO"
EXCEL_FILENAME = "API데이터_경기_득점_및_도움_기록.xlsx"
...
for name, ouid in player_map.items():
    ...
    while offset < 500:
        res = requests.get(
            f"https://open.api.nexon.com/fconline/v1/user/match?ouid={ouid}&matchtype=40&offset={offset}&limit=100",
            headers=headers,
        )
...
for i, m_id in enumerate(all_league_matches):
    ...
    while True:
        d_res = requests.get(
            f"https://open.api.nexon.com/fconline/v1/match-detail?matchid={m_id}",
            headers=headers,
        )
        if d_res.status_code == 429:
            print(
                "⏳ API 요청 한도 도달! 5초간 휴식 후 이 경기를 재시도합니다..."
            )
            time.sleep(5)
            continue
        break
    ...
    # 기준 시간 이전 경기 필터링
    if m_date_raw < TARGET_START_DATETIME:
        continue
    ...
    if ouid1 in league_ouids and ouid2 in league_ouids:
        ...
        for team in m_infos:
            ...
            for p in team.get("player", []):
                ...
                if assist_count > 0:
                    assisters.append(
                        f"{t_name}_{p.get('spId')}({assist_count}도움)"
                    )
...
# ------------------------------------------
# 4. DataFrame 생성, 정렬 및 1개 엑셀 파일(2개 시트)로 저장
# ------------------------------------------
...
# ✅ pd.ExcelWriter를 사용하여 1개의 파일에 시트를 나누어 저장
with pd.ExcelWriter(EXCEL_SAVE_PATH, engine="openpyxl") as writer:
    df_goal.to_excel(writer, sheet_name="득점 기록", index=False)
    df_assist.to_excel(writer, sheet_name="도움 기록", index=False)

print(
    f"\n🎉 모든 데이터 추출 및 저장 완료! (기준: {TARGET_START_DATETIME} 이후)"
)
print(f"📍 [엑셀 파일] 저장 완료: {EXCEL_SAVE_PATH}")
print("   └─ 📑 시트 1: 득점 기록")
print("   └─ 📑 시트 2: 도움 기록")